...
export default function Navbar({ currentRoute, onNavigate }: NavbarProps) {
  ...
  const navItems = [
    ...
    { label: 'Catálogo', route: 'catalogo' as AppRoute },
    { label: 'Marcas', route: 'marcas' as AppRoute },
    ...
    <>
      <header id="main-header" className="sticky top-0 z-50 w-full border-b border-slate-700 bg-[#0B1B32]/95 backdrop-blur-md text-white">
        ...
        <div id="transparency-banner-top" className="bg-[#061021] px-4 py-2 text-center text-xs font-semibold tracking-wide text-orange-400 md:text-sm border-b border-slate-800/80">
          <span className="inline-flex items-center gap-1.5 justify-center font-sans">
            <ShieldCheck className="h-4 w-4 text-emerald-500 shrink-0" />
            Catálogo Informativo Online • Nenhuma venda direta no site • Atendimento 100% via WhatsApp
          </span>
        </div>
        ...
        <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          >
            {logo && logo.trim() ? (
              ...
              <>
                <div className="relative flex h-10 w-10 items-center justify-center rounded bg-orange-500 overflow-hidden shadow-md shadow-orange-500/10">
                ...
                <div>
                  <span className="block font-sans font-black text-xl tracking-tight uppercase italic text-white group-hover:text-orange-400 transition-colors">
          ...
          {/* Desktop Navigation */}
          <nav id="desktop-nav" className="hidden lg:flex items-center gap-6 text-sm font-medium uppercase tracking-wide opacity-90">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative py-1.5 font-sans text-xs md:text-sm font-bold uppercase tracking-wider transition-colors hover:text-orange-500 ${
          ...
          <div className="hidden lg:flex items-center">
            <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-colors cursor-pointer"
            >
