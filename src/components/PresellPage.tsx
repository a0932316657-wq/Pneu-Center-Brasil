...
export default function PresellPage() {
  ...
  const [brandCards, setBrandCards] = useState<any[]>(() => {
    ...
    return getPresellBrandCards().filter(b => b.active).map(b => ({
      id: b.id,
      brand_name: b.brand_name,
      logo_url: b.logo_url || '',
      whatsapp_message: b.whatsapp_message || `Olá, gostaria de consultar pneus da marca ${b.brand_name}.`,
      active: b.active
    }));
  });
  ...
  const loadPresellDataLive = async () => {
    try {
      // 1. Fetch presell_settings
      const { data: settingsData, error: settingsError } = await supabase
        .from('presell_settings')
        .select('*');

      let fetchedSettings = getPresellSettings();
      ...
      if (!brandsError && brandsData && brandsData.length > 0) {
        const loadedBrands = brandsData.map((row: any) => ({
          id: row.id?.toString(),
        ...
        setBrandCards(loadedBrands);
      } else {
        // Fallback to general brands table
        const { data: standardBrands, error: stdBrandsError } = await supabase
          .from('brands')
          .select('*')
          .eq('active', true);
        if (!stdBrandsError && standardBrands) {
          const mapped = standardBrands.map((b: any) => ({
            id: b.id?.toString(),
            brand_name: b.name,
      ...
      console.warn('Erro ao carregar dados da Presell diretamente do Supabase:', err);
    }
  };
  ...
  const WHATSAPP_NUM = '5511995945993';
        ...
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900/40 border border-slate-850 rounded-2xl p-5 space-y-3 flex flex-col justify-start">
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                O único número oficial autorizado para negociações em nossa central de vendas é o (11) 99594-5993. Não responda a contatos estranhos.
              </p>
            </div>
        ...
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-950/40 p-6 rounded-2xl border border-slate-850/60 text-xs text-slate-400">
            ...
            <div className="space-y-2 md:border-l md:border-slate-800 md:pl-6">
              ...
              <p><span className="text-slate-500 font-medium font-sans">WhatsApp SAC:</span> (11) 99594-5993</p>
